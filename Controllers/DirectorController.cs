...
namespace Filmotheque.Controllers
{
    ...
    public class DirectorController : ControllerBase
    {
        ...
        public JsonResult GetAll([FromQuery] int page = 1, [FromQuery] int pageSize = 20)
        {
            ...
            List<Person> directorsInPage = _context.Directors.Skip((page - 1) * pageSize).Take(pageSize).ToList();
        ...
        {
            Person? a = _context.Directors.Find(id);
            if (a == null)
        ...
        {
            Person? oldDirector = _context.Directors.Find(id);
            if (oldDirector == null)
                return new JsonResult(NotFound($"Director of id {id} not found."));
